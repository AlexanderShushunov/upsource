...
RUN wget http://download.jetbrains.com/upsource/upsource-1.0.12566.zip  && unzip upsource-1.0.12566.zip  && mv Upsource /usr/local/upsource && rm upsource-1.0.12566.zip

#Export JAVA_HOME and setup hosts 
ENV JAVA_HOME /usr/lib/jvm/java-1.7.0-openjdk-amd64
RUN export PATH=~/bin:$JAVA_HOME/bin:$PATH
RUN /usr/local/upsource/bin/upsource.sh configure --listen-port 8080
EXPOSE 8080
